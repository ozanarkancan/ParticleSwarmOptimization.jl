...
__global__ void __update32(int n, float* px, float* pv, float* pbest, float* gbest,
    double w, double* l, double* h, double vmin, double vmax, double r1, double r2) {
    int i = threadIdx.x + blockIdx.x * blockDim.x;
    if(i < n){
        pv[i] = pv[i] * w + r1 * (pbest[i] - px[i]) + r2 * (gbest[i] - px[i]);
        pv[i] = pv[i] < vmin ? vmin : pv[i] > vmax ? vmax : pv[i];
        px[i] += pv[i];
        px[i] = px[i] < l[i] ? l[i] : px[i] > h[i] ? h[i] : px[i];
    }
}

__global__ void __update64(int n, double* px, double* pv, double* pbest, double* gbest,
    double w, double* l, double* h, double vmin, double vmax, double r1, double r2) {
    int i = threadIdx.x + blockIdx.x * blockDim.x;
    if(i < n){
        pv[i] = pv[i] * w + r1 * (pbest[i] - px[i]) + r2 * (gbest[i] - px[i]);
...
extern "C" {
    ...
    void update_64(int n, double* px, double* pv, double* pbest, double* gbest, double w, double* l, double* h, double vmin, double vmax, double r1, double r2) KCALL(__update64, n, px, pv, pbest, gbest, w, l, h, vmin, vmax, r1, r2);
    void update_32(int n, float* px, float* pv, float* pbest, float* gbest, double w, double* l, double* h, double vmin, double vmax, double r1, double r2) KCALL(__update32, n, px, pv, pbest, gbest, w, l, h, vmin, vmax, r1, r2);    
